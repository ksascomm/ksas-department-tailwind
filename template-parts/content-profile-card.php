...
<figure class="profile-card p-2 w-full md:w-1/3 m-0!">
	<div class="h-full px-6 py-4 mb-4">
		<div class="bg-cover rounded-sm h-80" style="background-image: url(<?php the_post_thumbnail_url( 'large' ); ?>)">
		</div>
		<figcaption>
		<h2 class="text-2xl!">
		...
		<?php if ( have_rows( 'custom_profile_fields' ) ) : ?>
			...
			while ( have_rows( 'custom_profile_fields' ) ) :
				the_row();
				?>
			<div class="my-2 text-lg font-bold leading-normal font-heavy text-primary"><span class="custom-title"><?php the_sub_field( 'custom_title' ); ?></span>&nbsp;<span class="custom-content"><?php the_sub_field( 'custom_content' ); ?></span></div>
			<?php endwhile; ?>
		<?php else : ?>
			<?php // No rows found! ?>
		<?php endif; ?>
		</figcaption>
		<blockquote class="relative flex flex-wrap items-center border-none quote">
		<svg class="absolute -top-4 -start-4 size-12 text-grey" width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
