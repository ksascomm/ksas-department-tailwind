<?php
/**
 * Template Name: People Directory (Columns)
 *
			...
			if ( ! empty( $filters ) && ! is_wp_error( $filters ) ) :
				...
				<div class="flex flex-col md:flex-row flex-wrap justify-start">	
					<?php foreach ( $filters as $filter ) : ?>
						<button class="all button bg-blue text-white text-lg hover:bg-blue-light hover:text-primary p-2" href="javascript:void(0)" data-filter=".<?php echo esc_html( $filter->slug ); ?>" class="selected"><?php echo esc_html( $filter->name ); ?></a>
					<?php endforeach; ?>
				</div>
			<?php endif; ?>
			<h4>
